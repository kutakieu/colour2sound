...
class MfccInversion(object):

    def __init__(self, fft_size=1024, spec_thresh=4, lowcut=500, highcut=15000, n_mel_freq_components=32, shorten_factor=10, start_freq=300, end_freq=8000):
        ...
        self.lowcut = lowcut  # Hz # Low cut for our butter bandpass filter
        self.highcut = highcut  # Hz # High cut for our butter bandpass filter
        # For mels
        ...
        self.mel_filter, self.mel_inversion_filter = self.create_mel_filter(fft_size=fft_size,
    ...
    def pretty_spectrogram(self, d, log=True, thresh=5, fft_size=512, step_size=64):
        ...
        """
        specgram = np.abs(self.stft(d, fftsize=fft_size, step=step_size, real=False,
                               compute_onesided=True))
    ...
    def invert_pretty_spectrogram(self, X_s, log=True, fft_size=512, step_size=512 / 4, n_iter=10):
        if log == True:
            X_s = np.power(10, X_s)

        X_s = np.concatenate([X_s, X_s[:, ::-1]], axis=1)
        X_t = self.iterate_invert_spectrogram(X_s, fft_size, step_size, n_iter=n_iter)
        return X_t


    def iterate_invert_spectrogram(self, X_s, fftsize, step, n_iter=10, verbose=False):
        """
        ...
        for i in range(n_iter):
            ...
            if i == 0:
                X_t = self.invert_spectrogram(X_best, step, calculate_offset=True,
                                         set_zero_phase=True)
            else:
            ...
            est = self.stft(X_t, fftsize=fftsize, step=step, compute_onesided=False)
    ...
    def invert_spectrogram(self, X_s, step, calculate_offset=True, set_zero_phase=True):
        """
        Under MSR-LA License
        Based on MATLAB implementation from Spectrogram Inversion Toolbox
        References
        ...
        wave = np.zeros((X_s.shape[0] * int(step) + size))
        # Getting overflow warnings with 32 bit...
        wave = wave.astype('float64')
        total_windowing_sum = np.zeros((X_s.shape[0] * int(step) + size))
        win = 0.54 - .46 * np.cos(2 * np.pi * np.arange(size) / (size - 1))
    ...
    def mel_to_spectrogram(self, mel_spec, mel_inversion_filter, spec_thresh, shorten_factor):
        """
    ...
    def sound2mfcc(self, data, rate):
        data = self.butter_bandpass_filter(data, self.lowcut, self.highcut, rate, order=1)

        wav_spectrogram = self.pretty_spectrogram(data.astype('float64'), fft_size=self.fft_size,
                                                           step_size=self.step_size, log=True, thresh=self.spec_thresh)

        mel_spec = self.make_mel(wav_spectrogram, self.mel_filter, shorten_factor=self.shorten_factor)

        return mel_spec

    def mfcc2sound(self, mfcc):

        mel_inverted_spectrogram = self.mel_to_spectrogram(mfcc, self.mel_inversion_filter,
                                                                    spec_thresh=self.spec_thresh,
                                                                    shorten_factor=self.shorten_factor)

        inverted_mel_audio = self.invert_pretty_spectrogram(np.transpose(mel_inverted_spectrogram),
                                                                     fft_size=self.fft_size,
                                                                     step_size=self.step_size, log=True, n_iter=10)
        _max = max(inverted_mel_audio)
        _min = min(inverted_mel_audio)
        if _max + _min > 0:
            norm = 1 / _max * 0x8000
        else:
            norm = 1 / _min * -0x8000
        # inverted_mel_audio *= 32767
        inverted_mel_audio *= norm
        inverted_mel_audio = inverted_mel_audio.astype("int16")
